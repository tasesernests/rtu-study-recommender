...
from scoring import score_all_programmes, breakdown_summary
...
[data-testid="stSidebar"] .stCaption { color: var(--neutral-600) !important; }
[data-testid="stSidebar"] h1,
[data-testid="stSidebar"] h2,
[data-testid="stSidebar"] h3 { color: var(--neutral-950) !important; }
...
[data-testid="stSidebar"] [data-baseweb="tag"] span { color: var(--neutral-700) !important; }
...
.stTabs [aria-selected="true"] {
    color: var(--neutral-950) !important;
    box-shadow: inset 0 -2px 0 var(--neutral-800) !important;
}
...
[data-testid="stExpander"] summary:hover { background: var(--neutral-100) !important; }
...
[data-testid="stMultiSelect"] [data-baseweb="tag"] span { color: var(--neutral-700) !important; }

/* ── SELECT SLIDER ────────────────────────────────────────────── */
[data-testid="stSlider"] [role="slider"] { background: var(--neutral-800) !important; }
...
def _section_label(icon: str, text: str):
    st.html(
        ...
        f'letter-spacing:0;color:#6b7280;margin:14px 0 6px 0;">{text}</div>',
    )
...
def render_sidebar(programmes: list[dict], taxonomy: dict) -> dict:
    with st.sidebar:
        # Brand header
        st.html("""
        <div style="padding:18px 8px 20px;">
          <div style="display:flex;align-items:center;gap:10px;">
            <div style="width:38px;height:38px;border-radius:8px;background:#111827;
                        color:#ffffff;display:flex;align-items:center;justify-content:center;
                        font-size:0.8rem;font-weight:800;">RTU</div>
            <div>
              <div style="font-size:1rem;font-weight:800;color:#111827;letter-spacing:0;">
                Recommender</div>
          ...
          <div style="height:3px;width:42px;background:#9f1d2f;border-radius:2px;margin-top:14px;"></div>
        </div>
        """)
        ...
        ai_rerank   = st.toggle(
            ...
            value=False,
            key="f_ai_rerank",
            help="Optional Gemini reranking based on the career goal text. Leave off for faster results.",
        )
        ...
        if n_saved > 0:
            st.html(
                f'<div style="background:#f9fafb;border:1px solid #d1d5db;border-radius:8px;'
                f'padding:10px 12px;margin-bottom:8px;">'
                f'<div style="color:#374151;font-weight:700;font-size:0.85rem;">'
                f'{n_saved} saved programme{"s" if n_saved != 1 else ""}</div></div>',
            )
            if st.button("Clear saved", key="clear_saved", use_container_width=True):
        ...
        st.markdown("---")
        st.html(
            f'<div style="color:#6b7280;font-size:0.7rem;text-align:center;line-height:1.9;">'
        ...
        ">
          ...
          <div>
            <div style="font-size:0.95rem;font-weight:700;color:#111827;
                        letter-spacing:0;">{title}</div>
            <div style="font-size:0.75rem;color:#6b7280;margin-top:1px;">{subtitle}</div>
          </div>
        </div>""",
    )
...
def render_student_form(programmes: list[dict], taxonomy: dict) -> dict | None:
    ...
    <div style="margin-bottom:18px;">
      <h2 style="margin:0;font-size:1.35rem;font-weight:800;color:#111827;
                 letter-spacing:0;">My Profile</h2>
      <p style="margin:5px 0 0;color:#6b7280;font-size:0.875rem;">
    ...
    with st.form("student_profile_form", clear_on_submit=False):
        ...
        _form_section("1", "Interests & Strengths",
        ...
        _form_section("2", "Study Preferences",
...
def _load_test_profile():
    ...
    try:
        ...
        st.toast(f"Could not load test profile: {e}")
...
def _explanation_cache_key(student: dict, programme: dict, breakdown: dict) -> str:
    payload = {
        "student": student,
        "programme_id": programme.get("id"),
        "programme_source": programme.get("source_file"),
        "score": breakdown.get("final_pct"),
        "matches": {
            "interests": breakdown.get("interests", {}).get("matched", []),
            "strengths": breakdown.get("strengths", {}).get("matched", []),
...
def render_results(student: dict, programmes: list[dict], filters: dict):
    ...
    if _filled == 0 and _flags == 0:
        st.warning(
            "**Your profile is empty.** Scores are rough estimates — they reflect "
    ...
    _ai_reranked = False
    if filters.get("ai_rerank"):
        with loading_spinner_context("Refining order from your career goal..."):
    ...
    if not top_results:
        st.warning(
            "No programmes found with the current filters. "
    ...
    # Confidence banner
    if top_score >= 75:
        b_color, b_bg, b_text = "#047857", "#f0fdf4", "High confidence"
    elif top_score >= 50:
        b_color, b_bg, b_text = "#b45309", "#fffbeb", "Good match"
    else:
        b_color, b_bg, b_text = "#b91c1c", "#fef2f2", "Partial match"

    st.html(f"""
    <div style="
      background:{b_bg};border:1px solid {b_color}30;
      border-left:4px solid {b_color};border-radius:8px;
    ...
    explanation_store = st.session_state.setdefault("ai_explanations", {})
    ...
    for rank_i, result in enumerate(top_results, start=1):
        ...
        explanation_key = _explanation_cache_key(student, prog, breakdown)
        cached_explanation = explanation_store.get(explanation_key, {})

        requested_explanation = render_result_card(
            rank=rank_i, programme=prog, score=score,
            breakdown=breakdown,
            ai_explanation=cached_explanation.get("text"),
            is_ai=bool(cached_explanation.get("is_ai")),
            summary=bd_summary,
            on_save=_save_programme,
        )
        if requested_explanation:
            with st.spinner(f"Generating explanation for programme #{rank_i}..."):
                explanation, is_ai = generate_ai_explanation(
                    student_profile=student,
                    programme=prog,
                    scoring_breakdown=breakdown,
                )
            explanation_store[explanation_key] = {"text": explanation, "is_ai": is_ai}
            st.session_state["ai_explanations"] = explanation_store
            st.rerun()
...
def _save_programme(prog_id: str, programme: dict):
    ...
    if prog_id in saved:
        ...
        st.toast("Programme removed from saved.")
    else:
        saved.add(prog_id)
        saved_data[prog_id] = programme
        st.toast(f"Saved: **{programme.get('name', prog_id)}**")
...
def render_compare_tab():
    ...
    if not saved_list:
        ...
        <div style="text-align:center;padding:48px 24px;color:#6b7280;">
          <h3 style="color:#374151;font-weight:700;margin:0 0 8px;">No saved programmes</h3>
    ...
    st.html(
        f'<h3 style="margin:0 0 20px;font-size:1.2rem;font-weight:700;color:#111827;letter-spacing:0;">'
        f'Comparison - {len(saved_list)} programme{"s" if len(saved_list) != 1 else ""}</h3>',
    )
...
def render_all_programmes_tab(programmes: list[dict]):
    ...
    with col_title:
        st.html(
            '<h3 style="margin:0 0 4px;font-size:1.1rem;font-weight:700;color:#111827;letter-spacing:0;">'
...
def main():
    ...
    with tab_search:
        ...
        if st.session_state.get("search_done") and st.session_state.get("student_profile"):
            ...
            <p style="margin:0 0 20px;color:#6b7280;font-size:0.875rem;">
              The three best matches with optional AI explanations
            </p>
            """)
            render_results(
